...
public class Main {
	...
	public static void main(String... args) throws IOException, ApiException, KeyManagementException, NoSuchAlgorithmException {
		...
		Api api = new Api.Builder(properties.getProperty("api.baseuri"))
				.withHttpClientConfig(new HttpClientConfig()
						                      ...
						                      .acceptAllCerts()
						                      .setUserAgent("rpi api connector/2.0.0")
				                     )
				.withOAuthv2(properties.getProperty("api.username"), properties.getProperty("api.password"))
				.build();
		...
		api.organisations().post(OrganisationSend.createPost("test orga").setTrialIssueTypes(Arrays.asList("123", "234"))).process(organisation -> {
			System.out.println(organisation);
			api.organisations().patch(organisation.getId(), OrganisationSend.createPatch().setValidUntil(OffsetDateTime.now().plusMonths(1)).setTrialIssueTypes(null)).process(System.out::println, printError());
		...
		scanPath.stats().get(new Filter().limit(5)).process(System.out::println, printError());
			...
			api.applications().get().ifOk(apps -> apps.stream().findFirst().ifPresent(app -> {
				api.application(app.getId()).scans().get().ifOk(scans -> scans.stream().findFirst().ifPresent(scan -> {
					api.application(app.getId()).scans().patch(scan.getId(), ScanSend.createPatch().setTags(Arrays.asList("TAG1", "TAG2"))).ifOk(newScan -> {
						System.out.println(newScan.getTags());
					});
					api.application(app.getId()).scans().patch(scan.getId(), ScanSend.createPatch().setTags(Collections.singletonList("TAG3"))).ifOk(newScan -> {
						System.out.println(newScan.getTags());
					});
					api.application(app.getId()).scans().patch(scan.getId(), ScanSend.createPatch().setTags(null)).ifOk(newScan -> {
						System.out.println(newScan.getTags());
					});


					api.application(app.getId()).scans().post(
							ScanPost.createPost(ScanPost.ScanSub.createPost("123").setAnalysisDepth(5))
